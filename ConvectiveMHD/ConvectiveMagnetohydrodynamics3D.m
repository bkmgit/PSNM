% A program to solve the 3D convective incompressible magnetohydrodynamic 
...
Rem = 10.0;      % Magnetic Reynolds number
...
theta=cos(xx).*cos(yy).*sin(zz);
f=cos(xx).*cos(yy).*sin(zz);
...
thetahat=fftn(theta);
thetax=ifftn(thetahat.*kxm);thetay=ifftn(thetahat.*kym);thetaz=ifftn(thetahat.*kzm);
...
for n=1:Nt
    ...
    thetaxold=thetax; thetayold=thetay; thetazold=thetaz;
    ...
    rhshatthetafix=(1/dt + 0.5*(k2xm+k2ym+k2zm)).*thetahat;
    ...
    while (chg>tol)
        ...
        nonlinwhat=fftn(nonlinw)+0.5*Ra*(thetahat+thetahatold);
        ...
        nonlinu = 0.25*((u+uold).*(thetax+thetaxold) + ...
                        (v+vold).*(thetay+thetayold) + ...
                        (w+wold).*(thetaz+thetazold));
                        
        nonlinuhat = fftn(nonlinu);

        thetahat = (rhshatthetafix-dt*nonlinuhat)...
                   ./(1-0.5*dt*(k2xm+k2ym+k2zm));

        thetatemp=theta;
        
        theta=ifftn(thetahat);
        thetax=ifftn(thetahat.*kxm);
        thetay=ifftn(thetahat.*kym);
        thetaz=ifftn(thetahat.*kzm);          
        
        chg=max(abs(utemp-u))   +max(abs(vtemp-v))   +max(abs(wtemp-w)) +...
            max(abs(bxtemp-bx)) +max(abs(bytemp-by)) +max(abs(bztemp-bz) + ...
            max(abs(thetatemp-theta)));
    end
    ...
    omegabx=bxy-byz; omegaby=bxz-bzx; omegabz=byx-bxy; 
    omegabtot=omegabx.^2+omegaby.^2+omegabz.^2;
    % Get real temperature field
    thetatot=real(theta);
    
    figure(1); clf; 
    subplot(3,1,1); title(['|Fluid omega|^2 ',num2str(t)]);
    H = vol3d('CData',omegatot,'texture','3D','XData',x,'YData',y,'ZData',z);
    xlabel('x'); ylabel('y'); zlabel('z'); colorbar;
    axis equal; axis square; view(3); 
      
    subplot(3,1,2); title(['|Magnetic omega|^2 ',num2str(t)]);
    H = vol3d('CData',omegabtot,'texture','3D','XData',x,'YData',y,'ZData',z);
    xlabel('x'); ylabel('y'); zlabel('z'); colorbar;
    axis equal; axis square; view(3); 

    subplot(3,1,3); title(['Temperature ',num2str(t)]);
    H = vol3d('CData',thetatot,'texture','3D','XData',x,'YData',y,'ZData',z);
    xlabel('x'); ylabel('y'); zlabel('z'); colorbar;
    axis equal; axis square; view(3); 
